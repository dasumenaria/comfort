    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->ContraVouchers->save($contraVoucher)) {
                ...
                return $this->redirect(['action' => 'add'.'?status=add']);
            }
            $this->Flash->error(__('The contra voucher could not be saved. Please, try again.'));
        }
		$Voucher = $this->ContraVouchers->find()->select(['voucher_no','transaction_date'])->where(['company_id'=>$company_id,'financial_year_id'=>$financialYear_id])->order(['voucher_no' => 'DESC'])->first();
		if($Voucher)
		{
			$voucher_no=$Voucher->voucher_no+1;
			if(!empty($status)){
				$old_transaction_date = $Voucher->transaction_date;
			}else{
				$old_transaction_date = date('d-m-Y');
			}
		}
		else
		{ 
        ...
        $this->set(compact('contraVoucher','voucher_no','ledgers','company_id','old_transaction_date'));
        $this->set('_serialize', ['contraVoucher']);
    }
