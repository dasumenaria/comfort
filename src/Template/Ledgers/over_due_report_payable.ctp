   ...
   <div class="row">
	    <div class="col-md-12">
			...
			<div class="box-header with-border hide_print">
                          <i class="fa fa-plus"></i>Outstanding Payable Report [Creditors Report]
                        </div>
						...
						<tbody><?php $sno = 1; $dbt=[];$cdt=[];
							if($run_time_date){
								...
								if(@$dbt[$reference_detail->ledger->id] > 0 || @$cdt[$reference_detail->ledger->id] >0){	
									...
									if(($reference_detail->total_debit > 0 && $reference_detail->total_credit == 0) || ($reference_detail->total_credit > 0 && $reference_detail->total_debit == 0) || $duebalance > 0 || ($reference_detail->total_debit > 0 && $reference_detail->total_credit > 0))
									{ ?>
										<tr>
											<td><?php echo $sno++; ?></td>
											<td>
											...
											<td><?= $reference_detail->ledger->supplier->mobile; ?></td>
