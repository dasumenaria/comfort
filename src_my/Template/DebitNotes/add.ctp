        ...
        submitHandler: function () {
			...
			$(document).on('change','.refType',function(){
				...
				if(type=='Against'){
					...
					currentRefRow.find('td:nth-child(2)').html('<?php echo $kk; ?>');
					currentRefRow.find('td:nth-child(5)').html('<?php echo $dd; ?>');
			...
			$(document).on('change','.ledger',function(){
				...
				if(openWindow=='party'){
					...
					windowContainer.html('<table width=90% class=refTbl><tbody></tbody><tfoot><tr style=border-top:double#a5a1a1><td colspan=2><a role=button class=addRefRow>Add Row</a></td><td><?php echo $total_input; ?></td><td valign=top><?php echo $total_type; ?></td></tr></tfoot></table>');
			...
			function renameMainRows(){
				...
				$('#MainTable tbody#MainTbody tr.MainTr').each(function(){
					...
					$(this).find('td:nth-child(2) select.ledger').select2().attr({name:'debit_note_rows['+i+'][ledger_id]',id:'debit_note_rows-'+i+'-ledger_id'});
